...
f.DEFINE_string("out_dir", "./exp/conv_mnist/bae_1000", "The path to the directory containing the experimental results")
f.DEFINE_string("prior_dir", "./exp/conv_mnist/priors", "The path to the directory containing optimized priors")
...
f.DEFINE_integer("samples_per_class", 10, "The number of samples for each class in the subset of training data")
...
f.DEFINE_float("likelihood_temp", 1., "Temperature for likelihood term")
f.DEFINE_float("prior_temp", 1., "Temperature for prior term")
...
f.DEFINE_bool("only_zero", False, "Whether or not using only samples of 0-th classes for tuning prior")
...
set_seed(FLAGS.seed)
data = MNIST(label=True)
if FLAGS.samples_per_class < 50:
    train_loader, test_loader = data.get_data_loaders(1000)
else:
    train_loader, _ = data.get_data_loaders(60000)
    _, test_loader = data.get_data_loaders(1000)
x, y = next(iter(train_loader))
xs = []
ys = []

if FLAGS.only_zero:
    xs.append(x[torch.where(y==0)[0][:(FLAGS.samples_per_class*10)], ...])
    x_subset = torch.cat(xs)
else:
    for class_ in range(10):
        xs.append(x[torch.where(y==class_)[0][:FLAGS.samples_per_class], ...])
    x_subset = torch.cat(xs)

# Create dataloader for the subset of training data
dataset_subset = TensorDataset(x_subset)
dataloader_subset = DataLoader(dataset_subset, batch_size=FLAGS.batch_size, shuffle=True)
...
xs = xs[torch.where(ys!=0)[0][:FLAGS.training_size], ...]
...
if FLAGS.train:
    ...
    for x in inf_loop(train_loader):
        ...
        x = x[0].float().to(device)
        sampler.zero_grad()
        log_prob, log_lik, log_prior = model.forward(x, n_data, FLAGS.likelihood_temp, FLAGS.prior_temp)
        loss = -log_prob
        loss.backward()
        sampler.step()
